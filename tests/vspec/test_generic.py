...
def test_ros2interface_timeseries_cli_default_schema(tmp_path):
    """End-to-end CLI smoke test for --timeseries with the built-in timestamp defaults.

    Verifies that --timeseries produces the four expected files per signal and
    that point-in-time services are NOT generated when --srv is omitted.
    """
    vspec = _write_vspec(tmp_path)
    out = tmp_path / "out"
    subprocess.run(
    ...
    # --srv was not passed, so point-in-time services must NOT be generated.
