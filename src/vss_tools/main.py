...
def check_overlay_directives(
    ...
    """
    Validates 'new:' directives on overlay nodes against the base spec.

    new: true  — the key must NOT exist in the base spec (creating a new node)
    new: false — the key MUST exist in the base spec (modifying an existing node)
    """
    ...
    for overlay_path in overlays:
        ...
        for key, value in overlay_data.items():
            if not isinstance(value, dict):
                continue
            new_flag = value.get("new")
            if new_flag is True and key in base_keys:
                raise OverlayDirectiveException(
                    f"Overlay '{overlay_path.name}': '{key}' declares 'new: true' "
                    f"but already exists in the base spec"
                )
            if new_flag is False and key not in base_keys:
                raise OverlayDirectiveException(
                    f"Overlay '{overlay_path.name}': '{key}' declares 'new: false' "
