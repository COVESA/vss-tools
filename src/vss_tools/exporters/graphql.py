...
def get_metadata_df(root: VSSNode) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    core_headers = ["fqn", "parent", "name", "type", "description", "comment", "deprecated"]
...
def get_instances_enums() -> Dict[str, graphene.Enum]:
    ...
    for fqn, row in branches_with_instances.iterrows():
        ...
        enums[fqn] = type(enum_name, (graphene.Enum,), enum_values)  # type: ignore
...
def get_description(fqn: str) -> str:
    description = ""
    if fqn in vss_branches_df.index:
        description = str(vss_branches_df.loc[fqn, "description"])
    elif fqn in vss_leaves_df.index:
        description = str(vss_leaves_df.loc[fqn, "description"])
        if vss_leaves_df.loc[fqn, "min"]:
            description += f"\n@min: {str(vss_leaves_df.loc[fqn, 'min'])}"
        if vss_leaves_df.loc[fqn, "max"]:
            description += f"\n@max: {str(vss_leaves_df.loc[fqn, 'max'])}"
        if vss_leaves_df.loc[fqn, "default"]:
            description += f"\n@default: {str(vss_leaves_df.loc[fqn, 'default'])}"
    return description


def create_gql_object_type(fqn: str) -> graphene.ObjectType:
    """Create a GraphQL object type for a given Fully Qualified Name (fqn) fqn the VSS."""
    # Select the children of the current branch
    ...
    if fqn in branches_with_instances.index:
        gql_fields["instance"] = Field(name="instance", type_=gql_instance_enums[fqn])

    # Create a GraphQL field for each leaf that belongs to the current branch
    for child_fqn, child_leaf_metadata_row in child_leaves.iterrows():
        ...
        field_type = None
        field_description = get_description(str(child_fqn))
        unit = child_leaf_metadata_row["unit"]
        allowed = child_leaf_metadata_row["allowed"]
        extra_args = {}

        if allowed == "":
            # No allowed values are specified, then use the scalars from datatype_map as the field type
            field_type = datatype_map[child_leaf_metadata_row["datatype"]]
            ...
            if datatype and is_array(datatype):
                field_type = graphene.List(allowed_enum)
            else:
                field_type = allowed_enum

        if unit:
            try:
                quantity_kind = dynamic_units[unit].quantity  # Quantity kind name as described in VSS
                enum_name = get_unit_enum_name(quantity_kind)  # GraphQL enum name for the quantity kind
                ...
                if unit_enum_value not in unit_enum._meta.enum.__members__:
                    raise GraphQLExporterException(
                        f"Unit enum value '{unit_enum_value}' not found in enum '{unit_enum}'"
                    )
                else:
                    extra_args["unit"] = graphene.Argument(type_=unit_enum, default_value=unit_enum_value)  # type: ignore

                gql_fields[field_name] = Field(
                    name=field_name, type_=field_type, description=field_description, args=extra_args
                )
            except GraphQLExporterException as e:
                log.error(e)
                sys.exit(1)
        else:
            gql_fields[field_name] = Field(name=field_name, description=field_description, type_=field_type)

        global mapping_leaves_df
        mapping_leaves_df.loc[child_fqn, ["gql_field", "in_gql_type"]] = [field_name, gql_type_name]  # type: ignore

    # Create a field for each sub branch and call the creation of the GraphQL type recursively
