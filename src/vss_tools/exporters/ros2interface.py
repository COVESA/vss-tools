...
def render_delete_timeseries_srv(
    ...
    Request:
        ...
        <ts_type> start_time_<suffix>   # used when mode == TIME_WINDOW
        <ts_type> end_time_<suffix>     # used when mode == TIME_WINDOW
        uint32 keep_latest              # used when mode == RETENTION_FLOOR
    Response:
        bool success
        string message
    ...
    # TIME_WINDOW fields (used when mode == TIME_WINDOW)
    for ros_type, suffix in _timestamp_suffixes(timestamp_schema):
        request.append(f"{ros_type} start_time_{suffix}  # used when mode == TIME_WINDOW (TIME_WINDOW)")
    for ros_type, suffix in _timestamp_suffixes(timestamp_schema):
        request.append(f"{ros_type} end_time_{suffix}  # used when mode == TIME_WINDOW (TIME_WINDOW)")
    # RETENTION_FLOOR field (used when mode == RETENTION_FLOOR)
    request.append("uint32 keep_latest  # used when mode == RETENTION_FLOOR: min most-recent samples to retain")
