...
Patterns can be specified as:
  ...
  - Explicit prefix:     regex:^Vehicle\.Body\..*$ ,  glob:*.Speed ,  fqn:Vehicle.Speed ,  name:Speed
...
try:
    ...
    VSSNode = object  # fallback for type hints only
...
@click.option(
    ...
    help=(
        ...
        "'Vehicle.Speed', 'Speed', '*.Speed', 'regex:^Vehicle\\.Body\\..*$', 'name:Speed', 'fqn:Vehicle.Speed'. "
        "Repeatable."
    ),
)
@click.option(
