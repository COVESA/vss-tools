...
def cli(
    ...
    if datatype_tree:
        ...
        # TODO: Check if this is what we want. What about types_out?
        if extend_all_attributes:
            data["x-ComplexDataTypes"] = types_data

    top_node_name = list(data.keys())[0]
    data = data.pop(list(data.keys())[0])
    data = {
    ...
    write_json(data, output, 2 if pretty else None)
